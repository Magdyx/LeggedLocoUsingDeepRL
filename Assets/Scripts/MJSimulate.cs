    ...
    unsafe void Start ()
    {
        ...
        for( int i=0; i<nobject; i++ )
        {
            ...
            if( objects[i] )
            {
				...
				if (objects [i].name == "left_hand") {
					objects [i].transform.localPosition = new Vector3 (100, 100, 100);
				}
				else
...
//			transform.position
//			int index;
//			MJP.GetBodyIndex ("left_hand", &index);
//			MJP.GetObjectState(index, &transform, &visible, &selected);
//			SetTransform()
////			MJP.GetOption(&opt);
